...
#今借到登录地址, post登录
login_url = 'http://www.51jt.com/wap/wechat/wx-login.jsp'
...
class WY(object):
    def __init__(self, unam, upasswd):
        ...
        self.headers = {
                ...
                'Accept': '*/*',
                'Referer': 'http://www.51jt.com/wap/wechat/wx-login.jsp',
                'Accept-Encoding': 'gzip, deflate',
                'Accept-Language': 'zh-CN,en-US;q=0.8'
}
        self.consume_info = mongo_connect()['WY']
        self.info_dict = {}
        self.sess = requests.session()

    """df4c77661c31b283f1033c4e3bd53683"""
    """321389c2d06f7969a1f53a2cd3ec92e9"""
    def login(self):
        #今借到输入密码以后用MD5加密两次
        passwd = md5(self.upasswd)

        payload = {"username":self.uname,
                    "pwd":passwd,
                    "retUrl": "http://www.51jt.com/wap/wechat/account.jsp",
                    "action": "signin"
        }
        response = self.sess.post(url=login_url, data=payload, headers=self.headers, verify=False)
        print(response.text)
...
if __name__ == '__main__':
    ...
    init = WY('18645145512', 'hwz5980279')
    init.login()
